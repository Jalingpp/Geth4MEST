...
func (w *worker) mainLoop() {
	...
	for {
		...
		case ev := <-w.chainSideCh:
			...
			avglatency = avglatency * totalTimes
			...
			defer func(tfile *os.File) {
				tfile.Close()
			}(tfile)
			//写字符串
			tfile.WriteString(strconv.Itoa(totalTimes) + " times commit, average latency: " + strconv.Itoa(avglatency) + "ms\n")
...
func writeToFile(txs []*types.Transaction) {
	...
	if len(txs) > 0 {
		...
		// btnum1, _ := strconv.ParseInt("0x10", 0, 64)
		// bytesType1 := []byte(strconv.FormatInt(btnum1, 10))
		// btnum2, _ := strconv.ParseInt("0x20", 0, 64)
		// bytesType2 := []byte(strconv.FormatInt(btnum2, 10))
		...
		for i := 0; i < len(txs); i++ {
			...
			// fmt.Print("mesttype:", w.current.txs[i].MESTType(), "bytesType1:", bytesType1, "bytesType2:", bytesType2, line)
		...
		// fmt.Println("!!!!miner/worker.go/commit: sleep 1s.")

		tempFile := "datafile.tmp"
		_, err = copyFile("datafileGeth", tempFile)
		if err != nil {
			panic(err)
		}
